var test_var;
if (!window.IS_DATE_RANGE_PICKER_LOADED || typeof window.IS_DATE_RANGE_PICKER_LOADED === 'undefined') {
    window.IS_DATE_RANGE_PICKER_LOADED = true;
    (function($) {
        $.fn.daterangepicker = function(settings) {
            ...
            var options = $.extend({
                    ...
                    latestDate: Date.parse("+1day"), //latest date allowed
            ...
            var datepickerOptions = {
                onSelect: function(dateText, inst) {
                    ...
                    if (
                        rp.find(".ui-daterangepicker-specificDate").is(".ui-state-active")
                    ) {
                    ...
                    if (rangeInput.length == 2) {
                        ...
                        rangeInput.val(
                            rangeA != rangeB ?
                            rangeA + " " + options.rangeSplitter + " " + rangeB :
                            rangeA
                        );
                    }
                    ...
                    if (options.closeOnSelect) {
                        if (!rp
                            .find("li.ui-state-active")
                            .is(".ui-daterangepicker-dateRange") &&
                            !rp.is(":animated")
                        ) {
                            hideRP();
                        }
            ...
            options.datepickerOptions = settings ?
                $.extend(datepickerOptions, settings.datepickerOptions) :
                datepickerOptions;
            ...
            if (rangeInput.size() == 2) {
                inputDateAtemp = moment(
                    rangeInput.eq(0).val(),
                    settings.momentConverter
                ).toDate();
                inputDateBtemp = moment(
                    rangeInput.eq(1).val(),
                    settings.momentConverter
                ).toDate();
                ...
                inputDateAtemp = moment(
                    rangeInput.val().split(options.rangeSplitter)[0],
                    settings.momentConverter
                );
                inputDateBtemp = moment(
                    rangeInput.val().split(options.rangeSplitter)[1],
                    settings.momentConverter
                );
                if (!inputDateBtemp.isValid()) {
                    inputDateBtemp = inputDateAtemp;
                } //if one date, set both
            }
            if (inputDateAtemp.isValid()) {
                inputDateA = inputDateAtemp.toDate();
            }
            if (inputDateBtemp.isValid()) {
                inputDateB = inputDateBtemp.toDate();
            }
            ...
            $.fn.restoreDateFromData = function() {
                if ($(this).data("saveDate")) {
                    $(this)
                        .datepicker("setDate", $(this).data("saveDate"))
                        .removeData("saveDate");
                }
                return this;
            };
            ...
            function positionRP() {
                ...
                rp.parent()
                    .css(side, val)
                    .css("top", riOffset.top + relEl.outerHeight());
            }
            ...
            function clickActions(el, rp, rpPickers, doneBtn) {
                if (el.is(".ui-daterangepicker-specificDate")) {
                    ...
                    rp.find(".range-start")
                        .saveDateToData()
                        .datepicker("setDate", options.earliestDate)
                        .css("opacity", 0)
                        .hide(400);
                    rp.find(".range-end")
                        .restoreDateFromData()
                    ...
                    rp.find(".range-end")
                        .saveDateToData()
                        .datepicker("setDate", options.latestDate)
                        .css("opacity", 0)
                        .hide(400);
                    setTimeout(function() {
                        doneBtn.fadeIn();
                    ...
                    var dateStart =
                        typeof el.data("dateStart") == "string" ?
                        Date.parse(el.data("dateStart")) :
                        el.data("dateStart")();
                    var dateEnd =
                        typeof el.data("dateEnd") == "string" ?
                        Date.parse(el.data("dateEnd")) :
                        el.data("dateEnd")();
                    rp.find(".range-start")
                ...
                .bind("constrainOtherPicker", function() {
                    if (options.constrainDates) {
                        ...
                        if ($(this).is(".range-start")) {
                            rp.find(".range-end").datepicker(
                                "option",
                                "minDate",
                                $(this).datepicker("getDate")
                            );
                        } else {
                            rp.find(".range-start").datepicker(
                                "option",
                                "maxDate",
                                $(this).datepicker("getDate")
                            );
            ...
            if (options.arrows && rangeInput.size() == 1) {
                var prevLink = $(
                    '<a href="#" class="ui-daterangepicker-prev ui-corner-all" title="' +
                ...
                var nextLink = $(
                    '<a href="#" class="ui-daterangepicker-next ui-corner-all" title="' +
                ...
                $(this)
                    ...
                    .wrap(
                        '<div class="ui-daterangepicker-arrows ui-widget ui-widget-header ui-helper-clearfix ui-corner-all"></div>'
                    )
                    .before(prevLink)
                    .before(nextLink)
                    .parent()
                    ...
                    .click(function() {
                        ...
                        var diff =
                            Math.abs(new TimeSpan(dateA - dateB).getTotalMilliseconds()) +
                            86400000; //difference plus one day
                        ...
                        rpPickers.find(".range-start, .range-end ").each(function() {
                            ...
                            $(this)
                                .datepicker("setDate", thisDate.add({
                                    milliseconds: diff
                                }))
                                .find(".ui-datepicker-current-day")
                                .trigger("click");
    ...
    (function() {
        ...
        g.unit = _.process(
            g.ctoken("minute hour day week month year"),
            function(s) {
                return function() {
                    this.unit = s;
                };
    ...
    var TimePeriod = function(
        ...
        if (arguments.length == 7) {
            ...
            if (orient == +1) {
                while (
                    d1 < d2 &&
                    d1.clone().addDays(Date.getDaysInMonth(d1.getYear(), d1.getMonth())) <
                    d2
                ) {
                    d1.addMonths(1);
                    this.months++;
